...
@section('title', 'Edit Pengumuman Siswa')
@section('page_title', 'Edit Pengumuman Siswa')
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Form Edit Pengumuman untuk Siswa</h3>
                </div>
                <div class="card-body">
                    <form action="{{ route('guru.student-announcements.update', $studentAnnouncement->id) }}" method="POST">
                        @csrf
                        @method('PUT') {{-- Penting untuk method PUT --}}

                        <div class="form-group">
                            <label for="title">Judul Pengumuman <span class="text-danger">*</span></label>
                            <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $studentAnnouncement->title) }}" required>
                            @error('title')
                        ...
                        <div class="form-group">
                            ...
                            <select name="kelas_id" id="kelas_id" class="form-control @error('kelas_id') is-invalid @enderror">
                                @foreach($kelasOptions as $id => $name)
                                    <option value="{{ $id }}" {{ old('kelas_id', $studentAnnouncement->kelas_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                            @error('kelas_id')
                            <span class="invalid-feedback" role="alert">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="message" id="message" class="form-control @error('message') is-invalid @enderror" rows="6" required>{{ old('message', $studentAnnouncement->message) }}</textarea>
                            @error('message')
                            <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
