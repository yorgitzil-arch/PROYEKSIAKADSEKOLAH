...
@section('page_title', 'Edit Penugasan Guru')
        ...
        <div class="col-md-8 offset-md-2">
            <div class="card card-outline card-warning">
                <div class="card-header">
                    <h3 class="card-title">
                        <i class="fas fa-edit mr-1"></i> Form Edit Penugasan Guru
                    </h3>
                </div>
                <form action="{{ route('admin.guru-assignments.update', $assignment->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                            ...
                            <div class="input-group">
                                ...
                                <select name="kelompok_mapel_filter" id="kelompok_mapel_filter" class="form-control">
                                    ...
                                    @foreach($kelompokMataPelajaran as $kelompok)
                                        <option value="{{ $kelompok }}" {{ (isset($assignment->mataPelajaran) && $assignment->mataPelajaran->kelompok == $kelompok) ? 'selected' : '' }}>{{ $kelompok }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="input-group">
                                ...
                                <select name="tahun_ajaran_id" id="tahun_ajaran_id" class="form-control @error('tahun_ajaran_id') is-invalid @enderror" required>
                                    ...
                                    @foreach ($tahunAjarans as $ta)
                                        <option value="{{ $ta->id }}" {{ old('tahun_ajaran_id', $assignment->tahun_ajaran_id) == $ta->id ? 'selected' : '' }}>{{ $ta->nama }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="input-group">
                                ...
                                <select name="semester_id" id="semester_id" class="form-control @error('semester_id') is-invalid @enderror" required>
                                    ...
                                    @foreach ($semesters as $s)
                                        <option value="{{ $s->id }}" {{ old('semester_id', $assignment->semester_id) == $s->id ? 'selected' : '' }}>{{ $s->nama }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="input-group">
                                ...
                                <select name="jurusan_filter" id="jurusan_filter" class="form-control @error('jurusan_filter') is-invalid @enderror">
                                    ...
                                    @foreach ($jurusans as $jurusan)
                                        <option value="{{ $jurusan->id }}" {{ old('jurusan_filter', (isset($assignment->kelas->jurusan_id) ? $assignment->kelas->jurusan_id : '')) == $jurusan->id ? 'selected' : '' }}>{{ $jurusan->nama_jurusan }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="input-group">
                                ...
                                <select name="kelas_id" id="kelas_id" class="form-control @error('kelas_id') is-invalid @enderror" required>
                                    ...
                                    @foreach ($kelas as $kls)
                                        <option class="kelas-option jurusan-{{ $kls->jurusan_id }}" value="{{ $kls->id }}" {{ old('kelas_id', $assignment->kelas_id) == $kls->id ? 'selected' : '' }} data-jurusan-id="{{ $kls->jurusan_id }}">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <select name="status_konfirmasi" id="status_konfirmasi" class="form-control @error('status_konfirmasi') is-invalid @enderror" required>
                                    ...
                                    <option value="Pending" {{ old('status_konfirmasi', $assignment->status_konfirmasi) == 'Pending' ? 'selected' : '' }}>Pending</option>
                                    <option value="Dikonfirmasi" {{ old('status_konfirmasi', $assignment->status_konfirmasi) == 'Dikonfirmasi' ? 'selected' : '' }}>Dikonfirmasi</option>
                                    <option value="Ditolak" {{ old('status_konfirmasi', $assignment->status_konfirmasi) == 'Ditolak' ? 'selected' : '' }}>Ditolak</option>
                                </select>
                    ...
                    <div class="card-footer">
                        <button type="submit" class="btn btn-primary float-right ml-2">
                            <i class="fas fa-save mr-1"></i> Perbarui Penugasan
                        </button>
...
@push('scripts')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function loadMataPelajaran(selectedKelompok, selectedMataPelajaranId = null) {
                ...
                $j.ajax({
                    ...
                    success: function(data) {
                        ...
                        // Pilih mata pelajaran yang sebelumnya dipilih (untuk edit form atau old() pada create)
            ...
            // Set dropdown filter kelompok sesuai assignment
            $j('#kelompok_mapel_filter').val(assignmentKelompok);
            
            // Muat mata pelajaran berdasarkan kelompok assignment dan pilih mata pelajaran yang sedang diedit
            loadMataPelajaran(assignmentKelompok, currentAssignmentMataPelajaran);
            ...
            if (initialJurusanFilter) {
                ...
                // Jika tidak ada filter jurusan awal, pastikan semua kelas terlihat
                allKelasOptions.show();
            }
